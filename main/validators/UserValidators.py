        ...
        def wrap(*args, **kwargs):
            if request.method in only:
                ...
                username_validator = Validator(name='username')._required(username)._unique(username,
                    ...
                    username)._minlen(username, 8)._maxlen(username, 20)

                if username_validator.errors:
                    errors['username'] = username_validator.errors

                display_name_validator = Validator(name='display_name')._required(display_name)

                if display_name_validator.errors:
                    errors['display_name'] = display_name_validator.errors
